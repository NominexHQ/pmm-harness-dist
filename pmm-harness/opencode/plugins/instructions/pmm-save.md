...
8. **Thread Routing Rules** — Apply these rules when thread files are active:
...
9. **Git Integration** — If `memory/.pmm-sync-only` exists, skip all git operations (this machine is a sync peer). Otherwise, run 'git add memory/', commit with meaningful message, and push if remote exists.
10. **Report Results** — Inform user of files written and git status.
