...
name: pmm-query
description: >
  Query memory — context-first recall with deep traversal across vectors, graph, and taxonomy
  when needed. Trigger on: "pmm-query", "/pmm-query", "query memory", "search memory",
  "recall", "what did we decide about", "find in memory", "look up", "what do we know about",
  or any request to search or recall information from memory files.
---
# pmm-query
